...
indefinitely for EOF. Prompt streams send the canonical terminal's additional
Ctrl-D after flushing non-newline input so Codex observes the actual EOF. Exec
transports send periodic WebSocket pings so quiet, long-running commands do not
get dropped by an idle connection timeout.
